...
export function useGetBalance({ address }: { address: Address }) {
  const { cluster: cluster }: useWalletUiCluster = new useWalletUiCluster (), { client } = useWalletUi ()
  return useQuery({
    ...
    queryFn: () =>
      client.rpc
        .getBalance(address)
        .send()
        .then((res: { value: any }) => res.value),
  })
}

export function useGetSignatures({ address }: { address: Address }) {
  const { cluster } = new useWalletUiCluster ()
  const { client } = new useWalletUi ()
...
export function useTransferSol({ address, account }: { address: Address; account: UiWalletAccount }) {
  ...
  const txSigner = new useWalletAccountTransactionSendingSigner ( account, cluster.id )
